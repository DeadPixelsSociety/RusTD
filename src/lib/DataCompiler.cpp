...
bool dc_compileCreep(TCreep* tc, data_byte data_bloc, int* data_pointer, int bloc_size)
{
	...
	if(!dc_compileFloat(&(stats.health_bonus), data_bloc, data_pointer, bloc_size))return false;
...
bool dc_extractCreep(TCreep** tc, data_byte data_bloc, int* data_pointer, int bloc_size)
{
	...
	Stats stats;
	float stats_health_base, stats_health_bonus, stats_health_regen;
	...
	if(!dc_extractFloat(&stats_health_bonus, data_bloc, data_pointer, bloc_size))return false;
	...
	stats.health_bonus = stats_health_bonus;
